...
namespace ImageFilters
{
    public class ImageOperations
    {
        ...
        /// <param name="X">Dimension on X-axis</param>
        /// <param name="Y">Dimension on Y-axis</param>
        public static byte[] Flatten(byte[,] Matrix, int X, int Y)
        {
            byte[] flatArray = new byte[X * Y];
            for (int i = 0; i < X; i++)
            {
                for (int j = 0; j < Y; j++)
                {
